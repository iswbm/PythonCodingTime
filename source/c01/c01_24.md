...
也许你看到这个标题，会失望，会觉得我怎么会写这么基础的文章？

不然，恰恰我觉得，这篇文章的内容是 Python 的高阶知识点，会深入地探讨会以真实案例讲解 Python import Hook 的知识点。
...
import 关键字的使用，可以说是基础中的基础。但对 `__import__` ，普通的开发者，可能就会比较陌生。
...
这里需要注意的是，在 3.4 版前，查找器会直接返回 加载器（Loader）对象，而在 3.4 版后，查找器则会返回模块规格说明（ModuleSoec），其中 包含加载器。 
...
我们有可能自定义查找器，但却很会有需要要自定义加载器。因此这边不会举例。

你只要知道我们在导入一个模块时，加载器到底做了哪些事情。
