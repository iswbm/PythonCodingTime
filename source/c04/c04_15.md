...
![](http://image.python-online.cn/20190419151948.png)
...
如果需要使用专业版，网上也有一些注册服务器使用，非常方便，缺点是过一段时间，可能就会失效。这里有有一种一劳永逸的方法，但是仅对早期的 PyCharm 版本（亲测 2017.1.5 可行，较新版本已经不支持了）有效，可以实现永久激活（到 2099 年，一定意义上是永久了吧）。

**注意**：这个方法仅限 Windows 用户（Mac 用户可直接跳过）

**第一步**：

下载jar包： 此jar包的目的就是让截获截止时间并骗过 PyCharm;

[点击下载 ](https://pan.baidu.com/s/1jHJUPwU)：密码:  53k4

**第二步**：

找到并进入你的 PyCharm 安装目录（以我的为例）：E:\Program Files\JetBrains\PyCharm 2017.1.5\bin
...
重启 PyCharm ，查看激活信息：Help -> About

![](http://image.python-online.cn/20190506150819.png)
