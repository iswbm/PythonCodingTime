...
如想要阅读全文，可以点击全文进行解锁：用微信扫描二维码关注我的个人公众号，发送 `more` ，将获取到的验证码填入如下文本框提交即可永久解锁本博客的所有干货文章。
...
这下就尴尬了。Sphinx 框架是固定的，我们无法对其进行定制修改。

还有什么方法呢？

我是否可以定制 OpenWrite 的 js 脚本整合在一个js文件中。
...
同时你如果之前是看过我写的教程，使用过我的中文检索插件。

由于这个插件是基于 Python 2.x 写的，现在我们切换成 3.x ，里面的代码也要对应修改。

改动也不大，只要把 `exts/smallseg.py`  这个文件里的 decode 相关的代码全部去年即可。若你嫌麻烦，也可以直接去我的 [Github](https://github.com/BingmingWong/PythonCodingTime) 找来用。

一切按照上面的步骤全部设置完成后，就可以从 readthedocs 上重新构建了。
