...
![](http://image.iswbm.com/20200711143644.png)
...
![Cookie + Session 方法](http://image.iswbm.com/20200707220426.png)
...
![Referer Check 图解](http://image.iswbm.com/20200705193118.png)
...
![](http://image.iswbm.com/image-20200707221742925.png)
...
![](http://image.iswbm.com/image-20200707222024941.png)
...
![JWT 工作图解](http://image.iswbm.com/20200705220524.png)
...
![](http://image.iswbm.com/20200711144042.png)
...
当然，如果你觉得你通过 `Referer Check` 、`加验证码` 等其他手段，已经可以保证不受 CSRF 攻击的威胁，此时你使用 JWT ，就可以选择使用 JWT + cookie HttpOnly，扼杀 XSS 攻击的可能。
...
前面我使用了 `pyjwt` 这个来生成 JWT ，事实上，这个库也可以用来验证 token。
...
如果不想验证签名及有效期，而只是想取下payload，只需加个`--no-verify` 参数即可

```shell
$ pyjwt --key="minggezuishuai" decode --no-verify {token}
```

更的详细使用方法，可以执行 `pyjwt --help` 学习或者前往官方文档：https://pyjwt.readthedocs.io/en/latest/index.html
...
![](http://image.iswbm.com/20200711141903.png)
