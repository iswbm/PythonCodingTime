...
这三个对象都是 `werkzeug` 里提供的，定义的 `local.py` 里。

## 3.5.1 Local

首先是 `Local` ，记得在以前的「并发编程系列」的第五篇线程的 `信息隔离` 的时候，提过了 `threading.local` ，它是专门用来存储当前线程的变量的。

而 Flask 里的 Local 和这它的是一个作用。

Local 的代码并不多，需要注意的就两点：

1. 变量是存储在`__storage__` 里的。这个玩意是个嵌套字典。

   ```python
   # 0 和 1 是线程 id
   self.__storage__[0][key] = value
   self.__storage__[1][key] = value
   ```

2. local 是需要被 `localmanager` 管理的，在请求结束后，会调用 `localmanager.cleanup()` 函数，其实是调用 `local.__release_local__ `  进行数据清理。是如何做到的呢，看下面这段代码。
...
`LocalStack` ，从命名上，可以知道，他是一个栈结构的对象。它的作用就是用来存储 ` Local` 对象的。的主要有两种上下文，`AppContext` 和 `RequestContext`。
