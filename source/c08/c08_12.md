...
那就要归功于 `routes` ，用来给服务内部定义url到具体函数的映射，这个和 deploy 提供的url到服务映射功能有相同也有不同，相同的是都是url到服务，不同的是deploy的层级更高一点，比如 `/v2/project_id/servers/`这个请求的url，deploy会将 `/v2` 的请求交给openstack_compute_api_v2，而再后面的活就全交给` routes `，routes 会将 `project_id/servers/` 的GET请求交给nova.api.openstack.compute.servers.Controller.index()，而 POST请求交给 create() 方法处理。这就是 routes.mappers 所提供的功能，根据path和请求方法，将请求映射到具体的函数上。

deploy 前面已经讲过，现在要来看看 routes。
...
mapper.resource("server", "servers",
                ...
                collection={'list_vm_state': 'GET', 'os_vmsum': 'GET'})
```
...
[route 举例总结](https://blog.csdn.net/bellwhl/article/details/8956088)