...
17.1）中，配置网络不是在 on_first_boot 函数里，而是在
stages.py:apply_network_config() 里。

|image15|

并且新版的cloudinit ，local 和 init
阶段都会去配置网络，区别在于，local阶段只写配置文件，而init阶段不仅写配置文件，还会启动网卡，将ip配置上。

centos 7.x ，因为会跳过，它是在 stages.py:apply_network_config
函数里，这是在 local 阶段写入配置文件，而在 init 阶段 启动网卡。
...
使用这种方式并不会将第一次配置的旧ip给清除掉。\ |image19|

这个问题，目前我只在CentOS6 中遇到过。可以通过修改代码让其先 ``ifdown``
再 ``ifup`` 就可以解决这个问题。\ |image20|
...
.. |image15| image:: http://image.python-online.cn/20190829141059.png
.. |image16| image:: http://image.python-online.cn/20190829113537.png
.. |image17| image:: http://image.python-online.cn/20190829141059.png
.. |image18| image:: http://image.python-online.cn/Fp1TeHSiIMIQoZygbW9VSfAagB_d
.. |image19| image:: http://image.python-online.cn/Fh-5SQ8qYjhJEKovI6LmIpabSy2c
.. |image20| image:: http://image.python-online.cn/20190430231812.png
.. |image21| image:: http://image.python-online.cn/20190430232309.png
.. |image22| image:: http://image.python-online.cn/20190429205735.png
.. |image23| image:: http://image.python-online.cn/20190430232911.png
.. |image24| image:: http://image.python-online.cn/20190623091911.png
.. |image25| image:: http://image.python-online.cn/20190708175813.png
